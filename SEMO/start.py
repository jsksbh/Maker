...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "COUDRA_1":
      try:
...
async def COUDRA_1(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
async def alive(client: Client, message):
    ...
    alive = f"""╭──── • ◈ • ────╮
么 [𝐬𝐨𝐮𝐫𝐜𝐞 𝐧𝐨𝐧𝐚](t.me/COUDRA_1) 💎 .
么  [𝒅𝒆𝒗 𝐚𝐬𝐮𝐮𝐭𝐞𝐲](t.me/COUDRA_1) 💎 .
╰──── • ◈ • ────╯
...
@Client.on_message(filters.command(["المطور ڪۅٛدࢪا","ڪۅٛدࢪا","المبرمج"], ""))
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="COUDRA_1")
...
@Client.on_message(filters.command(["فينوم","المطور ڪۅٛدࢪا","المطور ڪۅٛدࢪا"], ""))
...
async def booot(client: Client, message: Message):
    ...
    if not await is_served_chat(client, chat_id):
       try:
        ...
        await client.send_message(chat_id, f"تم رفع البوت بنجاح ايها العضو اللطيف مبرمج ڪۅٛدࢪا🥷")
        return 
       except:
          pass
    message.continue_propagation()
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["COUDRA_1"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["COUDRA_1"]:
   ...
   if message.from_user.username == "COUDRA_1":
...
async def tiillllli(client, message):
 if message.from_user.username in ["COUDRA_1"]:
   ...
   if message.from_user.username == "COUDRA_1":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة المطور ڪۅٛدࢪا 💎 .**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @COUDRA_1  💎 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @COUDRA_1  💎 .**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @COUDRA_1  💎 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["COUDRA_1"]:
         await message.reply_text("**♪ رتبتك هي : المطور ڪۅٛدࢪا 💎 .**")
         return
     if userr.username in ["COUDRA_1"]:
         await message.reply_text("**♪ رتبتك هي : المطور ڪۅٛدࢪا💎 .**")
         return
